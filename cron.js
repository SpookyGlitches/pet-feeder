...
let job = new CronJob("0 */2 * * * *", function () {
	...
	val1 = date1.toLocaleTimeString(["en-GB"], {
		hour: "2-digit",
		minute: "2-digit",
	});
	val2 = date2.toLocaleTimeString(["en-GB"], {
		hour: "2-digit",
		minute: "2-digit",
	});
	db.promise()
		.query(
			"SELECT p.id as id,p.arduino_uuid as uuid,s.time,s.duration FROM schedules s INNER JOIN pets p ON s.pet_id = p.id WHERE s.time BETWEEN ? AND ?",
			[val1, val2]
		)
		.then(([results, fields]) => {
